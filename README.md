...
- **预览黑屏很久（`startRunning` 很快但画面迟迟不出）**：常见原因包括：① **主线程被大量 `main.async` 堆积**；② **SwiftUI 每帧刷新时 `UIViewRepresentable.updateUIView` 反复 `removeFromSuperlayer` / 再插入预览层**，相当于每帧拆掉相机画面。当前实现将骨骼、叠字、结果合并为 **单次主线程刷新**、限制叠字/骨骼 **约 20fps** 更新，**仅在 `AVCaptureVideoPreviewLayer` 实例变化时**重新挂载预览层，并在改相机会话配置前 **`stopRunning()`**。
